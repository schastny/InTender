...
public class Conversion extends org.docx4j.convert.out.pdf.PdfConversion {
	...
	public void output(OutputStream os) throws Docx4JException {
		try {
			...
			org.w3c.dom.Document xhtmlDoc = org.docx4j.XmlUtils
					.neww3cDomDocument();
			...
			exporter.html(wordMLPackage, result, false,
					System.getProperty("java.io.tmpdir")); // false -> don't use
															// HTML fonts.
			...
			// See
			// https://xhtmlrenderer.dev.java.net/guide/users-guide-r7.html#xil_32
			org.xhtmlrenderer.extend.FontResolver resolver = renderer
					.getFontResolver();
			...
			while (fontMappingsIterator.hasNext()) {
				...
				PhysicalFont pf = wordMLPackage.getFontMapper()
						.getFontMappings().get(fontName);

				if (pf == null) {
					log.error("Document font " + fontName
							+ " is not mapped to a physical font!");
					continue;
				}

				embed(renderer, pf);
				// For any font we embed, also embed the bold, italic, and bold
				// italic substitute
				// .. at present, we can't tell which of these forms are
				// actually used, so add them all
				// bold, italic etc
			...
			//
			// xhtmlDoc = dbf.newDocumentBuilder().parse(new
			// File("C:\\Users\\jharrop\\workspace\\docx4all\\sample-docs\\comic.html"));
			// } catch (Exception e) {
			// e.printStackTrace();
			// }
	...
	protected void embed(org.xhtmlrenderer.pdf.ITextRenderer renderer,
		...
		try {
			if (pf.getEmbeddedFile().endsWith(".pfb")) {
				...
				// String afm =
				// fm.getPhysicalFont().getEmbeddedFile().substring(5,
				// fm.getPhysicalFont().getEmbeddedFile().length()-4 ) + ".afm";
				// // drop the 'file:'
				String afm = FontUtils.pathFromURL(pf.getEmbeddedFile());
				afm = afm.substring(0, afm.length() - 4) + ".afm"; // drop the
																	// 'file:'

				// Given the check in substituter, we expect to find one or the
				// other.
				File f = new File(afm);
				if (f.exists()) {
					// renderer.getFontResolver().addFont(afm, BaseFont.CP1252,
					// true,
					// FontUtils.pathFromURL(fm.getPhysicalFont().getEmbeddedFile()));
					// // drop the 'file:'
					try {
						renderer.getFontResolver().addFont(afm,
								BaseFont.IDENTITY_H, true,
								FontUtils.pathFromURL(pf.getEmbeddedFile())); // drop
																				// the
																				// 'file:'
						...
						log.error(pf.getName()
								+ " does not support UTF encoding");
					...
					pfm = pfm.substring(0, pfm.length() - 4) + ".pfm"; // drop
																		// the
																		// 'file:'
					...
					if (f.exists()) {
						try {
							...
							renderer.getFontResolver()
									.addFont(
											...
											BaseFont.IDENTITY_H,
											true,
											FontUtils.pathFromURL(pf
													.getEmbeddedFile())); // drop
																			// the
																			// 'file:'
						} catch (java.io.UnsupportedEncodingException uee) {
							uee.printStackTrace();
						...
						// Shouldn't happen.
						log.error("Couldn't find afm or pfm corresponding to "
								+ pf.getEmbeddedFile());
					}
				}
			} else {
				...
				renderer.getFontResolver().addFont(
						FontUtils.pathFromURL(pf.getEmbeddedFile()),
						BaseFont.IDENTITY_H, BaseFont.NOT_EMBEDDED);
			}
		} catch (java.io.IOException e) {

			/*
			 * [AWT-EventQueue-0] INFO packages.WordprocessingMLPackage -
			 * Substituting symbol with standardsymbolsl from
			 * file:/usr/share/fonts/type1/gsfonts/s050000l.pfb
			 * java.io.IOException: Unsupported font type at
			 * org.xhtmlrenderer.pdf
			 * .ITextFontResolver.addFont(ITextFontResolver.java:199)
			 * 
			 * .pfb not supported, even with iText 2.0.8
			 */
			e.printStackTrace();
			log.warn("Shouldn't happen - should have been detected upstream ... "
					+ e.getMessage() + ": " + pf.getEmbeddedFile());
		} catch (Exception e) {
			e.printStackTrace();
			log.error("Shouldn't happen - should have been detected upstream ... "
					+ e.getMessage());
		}
	}
}